...
export const api = {
  ...
  const response = await fetch(`${url}/api/fetch/products/customer?productCategory=${category}`);
  ...
  getProductById: async (id) => {
    try {
      const response = await fetch(`${url}/api/fetch/products/${id}`, {
        credentials: 'include'
      })
      return response
    } catch(error){
  ...
  getDetailsProductById: async(id) => {
    ...
    const response = await fetch(`${url}/api/product/details/${id}`);
  ...
  createOrder: async (userId, cartItems, shippingAddress) => {
    const order = await fetch(`${url}/api/v1/orders/place`,{
    	method: 'POST',
  ...
  getOrders: async (userId) => {
    const response = await fetch(`${url}/api/v1/orders/${userId}`, {
    	credentials: 'include'
    })
  ...
  getOrderById: async (orderId) => {
    ...
    const response = await fetch(`${url}/api/v1/order/confirmation/${orderId}`, {
    credentials: "include"
    })
  ...
  updateOrderStatus: async (orderId, status) => {
    ...
    const order = await fetch(`${url}/api/v1/admin/order-status`, {
    	method: 'PATCH',
  ...
  getAllOrders: async () => {
    const ordersData = await fetch(`${url}/api/v1/fetch/admin/orders`,{
	credentials: 'include'})
	console.log('orders . . . . @');
  ...
  addProduct: async (product) => {
    ...
    const response = await fetch(`${url}/api/upload/product`, {
      method: 'POST',
  ...
  console.log('ID & PRODUCT ... ', productId, updates);
    ...
    const response = await fetch(`${url}/api/update/fetch/product/${productId}`, {
    	method: 'PATCH',
  ...
  deleteProduct: async (productId) => {
    await fetch(`${url}/api/product/delete/${productId}`, {
    method: 'DELETE',
    credentials: 'include'
    })
    return true
  ...
  fetchProducts: async () => {
  	const productsData = await fetch(`${url}/api/fetch/products`,{
	credentials: 'include'		
	});
  ...
  adminOrderById: async(orderId) => {
  	const adminOrderData = await fetch(`${url}/api/v1/fetch/admin/order/detail/${orderId}`, {
  		credentials: 'include'
  	})
